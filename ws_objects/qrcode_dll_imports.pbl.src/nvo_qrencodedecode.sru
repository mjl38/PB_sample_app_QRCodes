$PBExportHeader$nvo_qrencodedecode.sru
forward
global type nvo_qrencodedecode from dotnetobject
end type
end forward
...
global nvo_qrencodedecode nvo_qrencodedecode

type variables

PUBLIC:
String is_assemblypath = ".\QRCodesDLLs\QREncodeDecodeLibrary.dll"
String is_classname = "QREncodeDecodeLibrary.QREncodeDecode"
...
public function string of_encodetexttoqrimage(string as_text,string as_imagefilename)
public function string of_decodeqrimagetotext(string as_imagefilename)
end prototypes
...
public function string of_decodeqrimagetotext(string as_imagefilename);
//*-----------------------------------------------------------------*/
//*  .NET function : DecodeQRImageToText
//*   Argument:
//*              String as_imagefilename
//*   Return : String
//*-----------------------------------------------------------------*/
...
/* Set the dotnet function name */
ls_function = "DecodeQRImageToText"
...
Try
	...
	/* Trigger the dotnet function */
	ls_result = This.decodeqrimagetotext(as_imagefilename)
	Return ls_result
Catch(runtimeerror re_error)
...
on nvo_qrencodedecode.create
call super::create
TriggerEvent( this, "constructor" )
end on

on nvo_qrencodedecode.destroy
TriggerEvent( this, "destructor" )
call super::destroy
end on

event constructor;// For the sake of using a "relative" path, we create this object directly in the application's Open event. At that moment, you can be sure the current folder
// points to the folder in which the EXE or PBL file with the application object is.

// In my case, we have the pbl with the application object in a foldername with the same name as the app's PBL.
// Therefore, when running from the IDE, I have to step one folder back and then into the \QRCodesDLLs folder.
// When NOT running from the IDE, I have the \QRCodesDLLs folder in the same folder where the EXE is.

// IDE:
// Commented for now, since this might not apply to everyone:
//if handle(getapplication()) = 0 then
// // running from the IDE:
//	is_assemblypath = "\..\QRCodesDLLs\QREncodeDecodeLibrary.dll"
//else 
// // running the EXE:
	is_assemblypath = ".\QRCodesDLLs\QREncodeDecodeLibrary.dll"
//end if
